    ...
    <div class="flex items-start w-full">
        ...
        <discussion-content-left class="relative w-full">
            <div class="mb-4 space-y-4">
                @if ($editing)
                    <input type="text" class="w-full p-2 mb-2 text-gray-700 border rounded-lg focus:outline-none"
                        wire:model="editingTitle">
                    <textarea class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none" rows="3"
                        wire:model="editingContent"></textarea>
                    <div class="flex justify-between mt-2">
                        <button wire:click="updateDiscussion"
                            class="px-4 py-2 text-white bg-green-500 rounded">@lang('discussions::messages.words.save')</button>
...
>>>>>>> 153a0f1 (Add initial categories work)
                ...
                @auth
                    <div class="mb-4">
                        ...
                        <button wire:click="answer"
                            class="px-4 py-2 mt-2 text-white bg-blue-500 rounded">@lang('discussions::messages.words.answer')</button>
                    </div>
                    @endif
                </div>
            ...
            <discussion-content-right class="{{ config('discussions.styles.sidebar_width') }} flex-shrink-0 text-sm ml-8">
                ...
                <p class="my-2 text-xs opacity-60">{{ $this->discussion->category_slug }}</p>
                <hr />
                ...
                @auth
                    ...
                    <div class="relative w-auto h-full my-2">
                        <button-notifications onclick="window.livewire.emit('toggleNotification', {{ $this->discussion->id }})"
                            class="flex items-center justify-center flex-shrink-0 h-full px-3 py-1 overflow-hidden text-sm space-x-1 font-medium border @if ($this->discussion->users->contains(auth()->user()->id)) {{ 'border-green-400 bg-green-50 text-green-600' }}@else{{ 'border-gray-300 bg-neutral-50 hover:bg-neutral-100' }} @endif cursor-pointer {{ config('discussions.styles.rounded') }}">
